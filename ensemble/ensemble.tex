\chapter{Representation Learning with Unlabeled Data}
\label{ch:ensemble}
%%%%%%%%%%%%%%%%%%%%%%% file template.tex %%%%%%%%%%%%%%%%%%%%%%%%%
%
...
% Transductive SVMs (TSVM)~\cite{Joachims:1999} (also its
% extentions \cite{neverhurt:icml11}) are among the most popular SSL
% methods, which use unlabeled data to encourage classifying boundaries
% to pass through regions where scarce samples are nearby. While the
% efficacy of the methods has been shown in many applications, they are
% suffering from lack of an efficient optimization method, which makes
% it problematic to apply them to large-scale datasets. Graph-based
% methods \cite{Zhu:Harmonic:03, Zhou:nips:04,
%   Fergus09,icml10:large:graph:ssl} work by propagating labels over a
% graph reflecting similarity among data samples, and efficient
% algorithms \cite{Fergus09,icml10:large:graph:ssl} have been developed
% for large-scale applications. One problem for these methods is that
% the power of discriminative learning is not leveraged, which degrades
% the performance especially for vision classification where very
% high-dimensional features are involved and classifying function is
% highly non-linear.
...
% different classes. We call this the \emph{exotic-inconsistency}
...
%  Solving SSL problems this way, EP addresses all the three
% aforementioned issues: (1) in addition to \emph{local-consistency}
% property, it also exploits \emph{exotic-inconsistency} property; (2)
% the learned new feature can be fed into any classifiers; and (3) it
% performs well for self-taught image classification, supported by
% experiments.
% We evaluate our method on 4 standard datasets:
% Scene-$15$~\cite{lazebnik:cvpr06}, LandUse-$21$~\cite{landuse21}
% Texture-$25$~\cite{UIUC:Texture}, and
% Caltech-$101$~\cite{FeiFei2004}. Experimental results show that our
% method outperforms competing methods for semi-supervised image
% classification, where the unlabeled images are from the same
% classes as the labeled ones.  The experiments also show that the
% method combines well with the previous SSL methods. Finally, we also
% test the method for self-taught image classification, where the unlabeled
% data is a random collection of images, and show that the learned image
% representation is superior to normal image features.
...
This paper is an extension of our ICCV paper \citep{dai:iccv13b} and
...
%   a good training set, e.g. some classes may
...
% classes.  The task is to discover these classes. Each image is
...
% % class labels given by all the base classifiers can be arbitrary,
% % which comes from the label inconsistency among all \wt sets.
% The whole pipeline of our method is sketched in
% Fig.~\ref{fig_pipeline}.  Though simple, our method outperforms
...
% Neural network-based approaches combine unsupervised and supervised
% learning by training feed-forward classifiers with an additional
% penalty from an auto-encoder or other unsu- pervised embedding of the
% data (Ranzato and Szummer, 2008; Weston et al., 2012). The Manifold
% Tangent Classifier (MTC) (Rifai et al., 2011) trains contrastive
% auto-encoders (CAEs) to learn the manifold on which the data lies,
% followed by an instance of TangentProp to train a classifier that is
% approximately invariant to local perturbations along the manifold. The
% idea of manifold learning using graph-based methods has most recently
% been combined with kernel (SVM) methods in the At- las RBF model
% (Pitelis et al., 2014) and provides amongst most competitive
% performance currently available.

% Semi-supervised learning (SSL) methods are designed to handle this
% spectrum of label information [26, 28]. They rely on the density
% structure of the data itself to propagate known labels to areas
% lacking annotations, and provide a natural way to incorporate labeling
% uncertainty. However, these methods require the explicit manipulation
% of an n × n Laplacian matrix (n being the number of data points), for
% example [2] notes: “our algorithms compute the inverse of a dense Gram
% matrix which leads to O(n3 ) complexity. This may be impractical for
% large datasets.”
...
\includegraphics[width=0.32\textwidth]{./ensemble/consis_fig_15-crop.pdf} & \hspace{-3mm}
\includegraphics[width=0.32\textwidth]{./ensemble/consis_fig_101-crop.pdf} & \hspace{-3mm} 
\includegraphics[width=0.32\textwidth]{./ensemble/consis_fig_event8-crop.pdf} \\ 
\scriptsize{\text{(a) Scene-15 (15,4485) }}  & \scriptsize{\text{(b) Caltech-101 (101, 8677)} }  & \scriptsize{\text{(c) Event-8 (8, 1574)} }  \\

\includegraphics[width=0.32\textwidth]{./ensemble/consis_fig_21-crop.pdf} & \hspace{-3mm}
\includegraphics[width=0.32\textwidth]{./ensemble/consis_fig_25-crop.pdf} & \hspace{-3mm} 
\includegraphics[width=0.32\textwidth]{./ensemble/consis_fig_indoor-crop.pdf} \\ 
...
Fig.~\ref{fig:mov1} shows the results on six datasets (Datasets and
features will be introduced in Sec.\ref{sec:experiments}). The results
...
\begin{figure*}[tb]
  \centering
$  \begin{array}{cccc}
 \includegraphics[width=0.47\textwidth]{./ensemble/ensemble_noise_fig_15-crop.pdf} & 
  \includegraphics[width=0.47\textwidth]{./ensemble/ensemble_noise_fig_event8-crop.pdf}  \\
...
% \begin{SCfigure}[1][h]
% \centering
%  \includegraphics[width=0.6\textwidth]{./ensemble_noise_fig_15-crop.pdf} 
%  \caption{\small{Classification accuracies of ensemble learning on the
%      the Scene-15 dataset~\cite{lazebnik:cvpr06}, for varying training
%      label noise $R$ and varying number of training trials
%      $T$. Experiments on other datasets obtain the same
%      trend. Ensemble learning is able to cancell out the deficiency of
%      the training sets even they are severe (e.g. $R=80\%$), given
%      that the deficiency modes are different or `orthogonal' and the
%      number of training sets are sufficiently large. The figure is
...
% from polynomial to linear in the number of images. Fig. 1 illustrates
...
% The dis$(.,.)$ in line $5$ represents the distance between two visual
% vectors, $L1$ distance metric in our implementation.
...
$\vect{s}^t = (\vect{s}_1^t, ..., \vect{s}_r^t) \in \mathbb{R}^{rn}$  %\tcc*[r]{Constructing $\mathcal{P}^t$}
  $\vect{c}^t = (\vect{c}_1^t, ..., \vect{c}_r^t) \in   \mathbb{R}^{rn} $\;
...
$\text{Sample } \mathcal{P}^t = \{(s_i^t, c_i^t)\}_{i=1}^{rn} \text{using Algo.~\ref{alg:max-min}}$ \;
...
by~\citet{Belkin:semiframe:2006}: $\gamma_A$ was set as the inductive
...
% As to features, while Algo.~\ref{alg:max-min} and
% Algo.~\ref{alg:ensemble:projection} use the same notation $\vect{x}$,
% we used GIST for Algo.~\ref{alg:max-min} and the concatenation of all
% the three features for Algo.~\ref{alg:ensemble:projection}. This is
% because Algo.~\ref{alg:max-min} needs a low dimensional feature to
% define neighborhoods, while Algo.~\ref{alg:ensemble:projection} needs
...
% feature to Algo.~\ref{alg:max-min} and
% Algo.~\ref{alg:ensemble:projection}.
...
\begin{figure*} 
  \centering
   $ \begin{array}{cccc}
\includegraphics[width=0.38\linewidth]{./ensemble/15_baseline-crop.pdf}& 
\includegraphics[width=0.38\linewidth]{./ensemble/21_baseline-crop.pdf}  \\ 
\scriptsize{\text{(a) Scene-15}} & \scriptsize{\text{(b) LandUse-21}}   \\
\includegraphics[width=0.38\linewidth]{./ensemble/25_baseline-crop.pdf}&
\includegraphics[width=0.38\linewidth]{./ensemble/building25_baseline-crop.pdf}   \\
\scriptsize{\text{(c) Texture-25}} & \scriptsize{\text{(d) Building-25}}  \\

\includegraphics[width=0.38\linewidth]{./ensemble/101_baseline-crop.pdf}& 
\includegraphics[width=0.38\linewidth]{./ensemble/event8_baseline-crop.pdf} \\
\scriptsize{\text{(e) Caltech-101}} & \scriptsize{\text{ (f) Event-8}} \\ 
\includegraphics[width=0.38\linewidth]{./ensemble/indoor_baseline-crop.pdf}&  
\includegraphics[width=0.38\linewidth]{./ensemble/stl10_baseline-crop.pdf}  \\ 
\scriptsize{\text{(g) Indoor-67}} & \scriptsize{\text{(h) STL-10}} \\
...
\begin{figure*} 
  \centering
   $ \begin{array}{cccc}
\includegraphics[width=0.38\linewidth]{./ensemble/Precision_15_transductive-crop.pdf}& 
\includegraphics[width=0.38\linewidth]{./ensemble/Precision_21_transductive-crop.pdf} \\ 
\scriptsize{\text{(a) Scene-15}} & \scriptsize{\text{ (b) LandUse-21}} \\ 
\includegraphics[width=0.38\linewidth]{./ensemble/Precision_25_transductive-crop.pdf}& 
\includegraphics[width=0.38\linewidth]{./ensemble/Precision_building25_transductive-crop.pdf}   \\
 \scriptsize{\text{(c) Texture-25}} & \scriptsize{\text{(d) Building-25}}  \\

\includegraphics[width=0.38\linewidth]{./ensemble/Precision_101_transductive-crop.pdf}& 
\includegraphics[width=0.38\linewidth]{./ensemble/Precision_event8_transductive-crop.pdf} \\ 
\scriptsize{\text{(e) Caltech-101}} & \scriptsize{\text{(f) Event-8}} \\
\includegraphics[width=0.38\linewidth]{./ensemble/Precision_indoor_transductive-crop.pdf}& 
\includegraphics[width=0.38\linewidth]{./ensemble/Precision_stl10_transductive-crop.pdf}  \\ 
 \scriptsize{\text{(g) Indoor-67}} & \scriptsize{\text{(h) STL-10}} \\
\end{array}$
\caption{Classification results of Ensemble Projection (EP) on the
  eight datasets, where three classifiers are used: $k$-NN, Logistic
...
\begin{tabular}{cc}
\toprule
\multicolumn{1}{c}{  \scriptsize{Methods}} \\ \midrule
\scriptsize{\emph{k}-NN}   \\
\scriptsize{\emph{k}-NN + \bf{EP} } \\
\scriptsize{LR}     \\
\scriptsize{LR + \bf{EP} }\\  
\scriptsize{SVMs}     \\
\scriptsize{SVMs + \bf{EP} }\\  
\scriptsize{HF}     \\
\scriptsize{HF + \bf{EP} }\\  
\scriptsize{AG}     \\
\scriptsize{AG + \bf{EP}} \\  
...
\begin{tabular}{cc}
\toprule
\multicolumn{1}{c}{ {\scriptsize{Scene-15}} } \\ \midrule
62.4  \\
75.6  \\
73.0  \\
\textbf{80.0}  \\
...
\begin{tabular}{cc}
\toprule
\multicolumn{1}{c}{\scriptsize{LandUse-21}} \\ \midrule
69.6  \\
75.6  \\
\underline{78.0}  \\
\textbf{80.6}  \\
...
\begin{figure*} 
  \centering
   $ \begin{array}{cccc}
\hspace{-1mm} 
\includegraphics[width=0.24\linewidth, height=36mm]{./ensemble/T-curve_1-crop.pdf} & \hspace{-2mm}
\includegraphics[width=0.24\linewidth, height=36mm]{./ensemble/r-curve_1-crop.pdf} & \hspace{-2mm}
\includegraphics[width=0.24\linewidth, height=36mm]{./ensemble/n-curve_1-crop.pdf} & \hspace{-2mm}
\includegraphics[width=0.24\linewidth, height=36mm]{./ensemble/m-curve_1-crop.pdf} \\
\hspace{-1mm} 
\includegraphics[width=0.24\linewidth, height=36mm]{./ensemble/T-curve_2-crop.pdf}&  \hspace{-2mm}
\includegraphics[width=0.24\linewidth, height=36mm]{./ensemble/r-curve_2-crop.pdf} & \hspace{-2mm}
\includegraphics[width=0.24\linewidth, height=36mm]{./ensemble/n-curve_2-crop.pdf}&  \hspace{-2mm}
\includegraphics[width=0.24\linewidth, height=36mm]{./ensemble/m-curve_2-crop.pdf} \\
...
Fig.~\ref{fig:mrnt} shows the results over a range of their
...
% \emph{exotic-inconsistency} assumption, thus degrading the quality of
...
\begin{figure*} 
  \centering
   $ \begin{array}{cccc}
\hspace{-2mm}
\includegraphics[width=0.35\linewidth, height=46mm]{./ensemble/Model-curve_1-crop.pdf} &  \hspace{-4mm}
\includegraphics[width=0.12\linewidth, height=42mm]{./ensemble/Model-legend1_crop.pdf} &  
\includegraphics[width=0.35\linewidth, height=46mm]{./ensemble/Model-curve_2-crop.pdf} &  \hspace{-4mm}
\includegraphics[width=0.12\linewidth, height=42mm]{./ensemble/Model-legend2_crop.pdf} \\
\end{array}$
...
% Fig.~\ref{fig:feature} show that all the learned features perform
...
\begin{figure*} 
  \centering
   $ \begin{array}{cccc} 
   \hspace{-1mm}
\includegraphics[width=0.31\linewidth, height=40mm]{./ensemble/self_stl10_knn-crop.pdf}& 
   \hspace{-1.5mm}
\includegraphics[width=0.31\linewidth, height=40mm]{./ensemble/self_stl10_LR-crop.pdf}& 
   \hspace{-1.5mm}
\includegraphics[width=0.31\linewidth, height=40mm]{./ensemble/self_stl10_SVM-crop.pdf} \\
\scriptsize{\text{(a) KNN}}  & \scriptsize{\text{(b) LR}} & \scriptsize{\text{(c) SVMs}}
\end{array}$
...
\multicolumn{1}{c}{\scriptsize{Methods}} \\ \midrule
\scriptsize{$k$-means} \\  
\scriptsize{$k$-means \bf{+ EP}} \\    
\scriptsize{Spectral Clustering} \\
\scriptsize{Spectral Clustering \bf{+ EP}} \\
\bottomrule
\end{tabular}
...
% projection, as we have already seen in Sec.~\ref{sec:self} that the
