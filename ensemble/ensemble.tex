...
% exploit this assumption~\citep{Joachims:1999, Zhou:nips:04,
...
% methods~\citep{SemiSVM, Joachims:1999, SemiBoost, SemiForest} have
...
% algorithms to leverage the structure of unlabeled data~\citep{SemiSVM,
...
The rest of this chapter is organized as follows. Section~\ref{en:sec:related}
...
% \etal \citep{Pietro_2010}. They also propose a framework for learning
% discriminative classifiers while clustering the data, called
% regularized information maximization.  Yet, the method still
% significantly differs from ours.  In particular, \citep{Pietro_2010}
...
% similarities with the method of Lee and Grauman \citep{Lee11cvpr}. They
% proposed to use \emph{curriculum learning}
% (CL)~\citep{Curriculum_learning} for unsupervised object discovery.  CL
...
% \citep{Curriculum_learning}. Besides technical differences, the main
% benefit of our work compared to \citep{Curriculum_learning} is that we
...
close to ours is 
% Dai \etal~\citep{dai:eccv12b}.  They presented
...
hierarchies. For scene class discovery, \citep{dai:cvpr10}
...
% Self-taught learning~\citep{self-taught:icml07}, where sparse coding is
...
Figure~\ref{en:fig:mov1} shows the results on six datasets (Datasets and
features will be introduced in Section~\ref{en:sec:experiments}). The results
...
\scriptsize{\text{(a) Scene-15}}  & \scriptsize{\text{(b) Event-8}}
  ...
  \caption{Classification accuracy of ensemble learning on the the
    Scene-15 dataset~\citep{scene-15} and the Event-8
...
%      the Scene-15 dataset~\citep{scene-15}, for varying training
...
% and EL has already been shown in the field~\citep{Fern03RP}. A random
...
$\text{Sample } \mathcal{P}^t = \{(s_i^t, c_i^t)\}_{i=1}^{rn} \text{using Algorithm~~\ref{en:alg:max-min}}$ \;
...
\label{en:alg:ensemble:projection}
\end{algorithm}


\section{Experiments}
\label{en:sec:experiments}
...
\item Scene-15~\citep{scene-15}: $15$ scene classes with both
...
% dataset~\citep{imagenet:cvpr09} to evaluate our method on the task of
...
% Coding~\citep{LLC:cvpr10} with default parameter settings were employed
...
% coding and pooling~\citep{devil:bmvc11}.
...
% As to features, while Algorithm~~\ref{en:alg:max-min} and
% Algorithm~~\ref{en:alg:ensemble:projection} use the same notation $\vect{x}$,
% we used GIST for Algorithm~~\ref{en:alg:max-min} and the concatenation of all
% the three features for Algorithm~~\ref{en:alg:ensemble:projection}. This is
% because Algorithm~~\ref{en:alg:max-min} needs a low dimensional feature to
% define neighborhoods, while Algorithm~~\ref{en:alg:ensemble:projection} needs
...
% feature to Algorithm~~\ref{en:alg:max-min} and
% Algorithm~~\ref{en:alg:ensemble:projection}.
...
Figure~\ref{en:fig:mrnt} shows the results over a range of their
...
\end{array}$
\caption{Comparison of our learned feature to the CNN feature~\citep{deep:bmvc14}, with different LR models.}
  \label{en:fig:classifiers}
\end{figure*}
...
% Figure~\ref{en:fig:feature} show that all the learned features perform
...
We follow existing work \citep{Sivic05b,
  Tuytelaars_UnsupervisedSurvey, dai:cvpr10, dai:eccv12b, fakton:eccv12} and
...
% fields~\citep{dai:eccv12b,cvpr13:graph_location} recently, and we are
...
% projection, as we have already seen in Section~~\ref{en:sec:self} that the
