...
% inputs. Since MISR are more restricted, this work focuses on SISR.
...
% employs Embedding technique for the regularization; \citep{JOR:EG15}
...
% speeded up later by \citep{}; \citep{} learns coupled
% dictionaries for LR and HR patches via forcing shared encoding;
% \citep{, Timofte-ACCV-2014} combine the two ideas for
% efficient systems by learning local regression functions for the atoms
% of the dictionary. \citep{} exploits the efficiency of random
% forest for the task by defining a novel information gain computed with
% both LR and HR patches. The exception is the work
% SRCNN~\citep{} which learns the transformation from LR
% images to their HR version via convolutional neural network.

There is also a survey paper on ISR~\citep{ISR:survey}, providing an excellent summary of  
the theory and applications of ISR. \citep{seven:way} exploits 
seven ways to improve the performance of general example-based ISR methods.  
The work most relevant to ours is~\citep{SR:benchmark}, where different
ISR methods are evaluated. While sharing similarities, the two methods
still differ significantly. \citep{SR:benchmark} conducted user
...
such as \citep{face:SR08} for face recognition and \citep{Jing_2015_CVPR}
...
AUC values of boundary detection via CBD~\citep{isola2014crisp} on the super-resolved images by 
...
\etal~\citep{Zeyde-CS-2012}, ANR~\citep{Timofte-ICCV-2013},
A+~\citep{Timofte-ACCV-2014}, SRCNN~\citep{Dong-ECCV-2014}, 
JOR~\citep{JOR:EG15}, and SRF~\citep{SR_forest}, followed by the evaluation on the four vision
...
trained with the same training dataset from~\citep{Yang-TIP-2010},
...
with four standard perceptual criteria~\citep{SR:benchmark}, namely
...
systems~\citep{Martin-ICCV-2001, isola2014crisp}. This section
evaluates the usefulness of ISR methods for BD.  We use Crisp Boundary
Detection~\citep{isola2014crisp} (CBD), which is an unsupervised
...
evaluated on the BSDS300 dataset~\citep{Martin-ICCV-2001}. The whole
...
Benchmark~\citep{Martin-ICCV-2001}.
...
% Fig.~\ref{sr:fig:ed_method} illustrates the PR curves
...
% when the other value is fixed. As Figure~\ref{sr:fig:ed_method}
...
% accuracy of detected boundaries. As Table~\ref{sr:tab:ed} demonstrates,
...
% Figure~\ref{sr:fig:ed_factor} demonstrates.
...
%   corresponding detected boundary maps by CBD~\citep{isola2014crisp}
...
\hline
%\multicolumn{2}{|c||}{Benchmark} &Bicubic&SF&Zeyde~\citep{Zeyde-CS-2012}&GR~\citep{Timofte-ICCV-2013}& ANR~\citep{Timofte-ICCV-2013}& NE+LS~\citep{Timofte-ICCV-2013}& NE+NNLS~\citep{Timofte-ICCV-2013}& NE+LLE~\citep{Timofte-ICCV-2013}& SRCNN~\citep{Dong-ECCV-2014}&\textbf{A+}~\citep{Timofte-ICCV-2013}\\
...
% \multicolumn{2}{|c|}{ } & &\citep{Zeyde-CS-2012}& \citep{Timofte-ICCV-2013}& \citep{Dong-ECCV-2014} & \citep{Timofte-ACCV-2014} &\citep{JOR:EG15} & \\
\hline\hline
...
number of methods proposed~\citep{texton-forests, superpixel:eccv14, Long_2015_CVPR}.  We follow
the footsteps of most previous works on semantic image segmentation and choose the
standard MSRC-21~\citep{Shotton:2006:ECCV} dataset for the
evaluation. 
MSRC-21 consists of 591 images of 21 semantic
categories. For the segmentation method, we employ the recent
approach~\citep{superpixel:eccv14} for its simplicity in order to better 
show the influence of ISR. \citep{superpixel:eccv14}
...
The classification performance is listed in Table~\ref{sr:tab:dr}. The table demonstrates that ISR methods do improve the
...
% \begin{tabular}{|l|c|cccccc|c|c|c|}
% \hline
%  \multicolumn{2}{|c|}{BSDS300} & Bicubic & Zeyde~\etal\citep{Zeyde-CS-2012} & ANR\citep{Timofte-ICCV-2013} & SRCNN\citep{Dong-ECCV-2014} & A+\citep{Timofte-ACCV-2014} & JOR\citep{JOR:EG15} & Original \\
% % \multicolumn{2}{|c|}{FDDB} & Bicubic & Zeyde~\etal & ANR & SRCNN & A+ & JOR & Original \\
% % \multicolumn{2}{|c|}{THR = 0.5} & &\citep{Zeyde-CS-2012}& \citep{Timofte-ICCV-2013}& \citep{Dong-ECCV-2014} & \citep{Timofte-ACCV-2014} &\citep{JOR:EG15} & \\
% \hline\hline
...
% lots of time.  It has proven by many works~\citep{face:SRTIP,
...
% The Face Detection Data Set and Benchmark (FDDB)~\citep{fddbTech} is
...
% Viola-Jones algorithm~\citep{viola2001rapid} on each of the
...
% Table~\ref{sr:tab:fr} demonstrates continuous and discrete true positive
...
% different values of THR in Fig.~\ref{sr:fig:fr}, from which it can be
